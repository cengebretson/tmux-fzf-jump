...
If [tmux-attention](https://github.com/cengebretson/tmux-attention) is installed, a matching summary icon replaces the normal window icon and each pane's own status icon replaces its normal pane icon. Attention states take precedence; otherwise an active agent turn renders the working icon. Window rows always retain their native tmux window name, while pane rows show a pane's project while its agent is active. Multiple agents in separate panes are displayed independently. The integration is optional, and the normal window or pane icon remains when neither state is active or the matching state icon is configured as empty.
